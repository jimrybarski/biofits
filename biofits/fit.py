...
def fit_hyperbola(concentrations, signals):
    ...
    (yint, delta_y, kd), covariance = curve_fit(hyperbola, concentrations, signals, bounds=((yint_min, delta_y_min, 0.0),
...
def fit_quadratic(concentrations, signals):
    ...
    yint_min, yint_max = (0.0, np.inf) if intercept > 0 else (-np.inf, 0.0)
    delta_y_min, delta_y_max = (0.0, np.inf) if slope > 0 else (-np.inf, 0.0)
    (yint, delta_y, kd, constant), covariance = curve_fit(quadratic, concentrations, signals,
                                                          bounds=((yint_min, delta_y_min, 0.0, 0.0),
